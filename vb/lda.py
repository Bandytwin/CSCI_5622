...
class VariationalBayes:
    ...
    def new_phi(gamma, beta, word, count):
        ...
        b_vec = beta[:,word]
        d1 = digam(gamma)
        d2 = ones(len(gamma)) * digam(numpy.sum(gamma))
        phi_n = b_vec * numpy.exp(d1 - d2)
        phi = phi_n / numpy.sum(phi_n)
    ...
    def m_step(self, topic_counts):
        ...
        totals = numpy.sum(topic_counts,1)
        [row, col] = numpy.shape(topic_counts)
        totals_mat = numpy.transpose( numpy.matlib.repmat( totals, col, 1) )
        new_beta = topic_counts / totals_mat
